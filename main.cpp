...
//             auto req = std::make_tuple(string("Instrument on day"), name, d,
//             0); if (find(requirements.begin(), requirements.end(), req) !=
//                 requirements.end())
//                 continue;
...
//         mapActions[desc].push_back(make_tuple("value in column", c, 0,
//         value)); mapActions[desc].push_back(make_tuple("value in row", r, 0,
//         value)); mapActions[desc].push_back(make_tuple("slot filled", r, c,
//         "0")); mapActions[desc].push_back(
...
void Sudoku25() {
    using R =
        tuple<string /*desc*/, int /*row*/, int /*col*/, char /*value*/>;
    using A =
        tuple<string /*desc*/, int /*row*/, int /*col*/, char /*value*/>;
    using P = vector<vector<char>>;
    ...
    for (char ch = 'A'; ch <= 'Y'; ++ch) {
        vecValues.push_back(ch);
    }
    P prefills(vecValues.size(), vector<char>(vecValues.size(), '.'));
    istringstream sin(R"(..I..N....QR.....V.LWU..D
DX.P.EFU..T.N.I..W...O..B
..NVSC......KOF..A.......
T..OEV..WKU.JLX...Y..G.P.
...RLM.OGX..BDPI..NH..K..
WA...IG.P...QCT.B....H...
HSDX.KOQ.EA...W..IL.YTPUG
R.L.C..NM.HG..S....J.WB.X
.JUT.D.B.CR..M..G..WAIF.Q
.YG...WT...U.K....D.SC.VJ
..M....E.ROW.IV.SGC..A..P
...BK.N.VMS...HD.EJ..QUT.
EGT......F..D.J.WMR.O...V
.VQ.FXD.....PT....BO....Y
UOXW.ASH.IY..G..P.T.JREB.
.K.G..Q...J..PDCA..U..O.R
..JU.F.CB...E.GS.K.TP.I.M
F....G..YS..T...H.P..V.A.
....MU....C.S..Q.....KJF.
.T...PL.JDF.R.M..NEVGB.WS
J.V.....E.GBW.K.IFANUY...
.M..Q.I..YNF...L..WK....O
A.RDGJ...QIS.HOP.YVBEF...
Y.O...BP...MXJAR..H.K..Q.
.IW...C.OVP.L.YG.XM...H.A)");
    ...
    auto IsValidAction = [&](const P& prefills, int r, int c, char value) {
    ...
    auto FillRequirements = [&](const Solver<R>::ActionID& desc, int r, int c,
                                char value) {
        int nBoxIdx = r / nBoxSize * nBoxSize + c / nBoxSize;
    ...
    //SudokuSolver<RowSize, nBoxSize> s(prefills, requirements, mapActions);
    Solver<R> s(requirements, mapActions, {}, true);
    auto t4 = hc.now();
